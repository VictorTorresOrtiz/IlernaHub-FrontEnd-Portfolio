<!DOCTYPE html>
<html lang="en">

  <head>
    <!-- ***** METAS ***** -->
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <link rel="icon" type="image/x-icon" href="assets/images/apps_assets/favicon.png">
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@100;200;300;400;500;600;700;800;900&display=swap" rel="stylesheet">

    <title>Friends | IlernaHub</title>

    <!-- ***** Favicon ***** -->
    <link rel="icon" type="image/x-icon" href="assets/images/apps_assets/favicon.png">

    <!-- ***** Bootstrap CSS ***** -->>
    <link href="vendor/bootstrap/css/bootstrap.min.css" rel="stylesheet">


    <!-- ***** CSS CORE ***** -->
    <link rel="stylesheet" href="assets/css/fontawesome.css">
    <link rel="stylesheet" href="assets/css/style.css">
    <link rel="stylesheet" href="assets/css/owl.css">
    <link rel="stylesheet" href="assets/css/animate.css">
    <link rel="stylesheet"href="https://unpkg.com/swiper@7/swiper-bundle.min.css"/>

    <!-- ***** JS ***** -->
    <script defer src="vendor/jquery/jquery.min.js"></script>
    <script defer src="vendor/bootstrap/js/bootstrap.min.js"></script>
    <script defer src="assets/js/isotope.min.js"></script>
    <script defer src="assets/js/owl-carousel.js"></script>
    <script defer src="assets/js/tabs.js"></script>
    <script defer src="assets/js/popup.js"></script>
    <script defer src="assets/js/custom.js"></script>

  </head>

<body>

  <!-- ***** Preloader Start ***** -->
  <div id="js-preloader" class="js-preloader">
    <div class="preloader-inner">
      <span class="dot"></span>
      <div class="dots">
        <span></span>
        <span></span>
        <span></span>
      </div>
    </div>
  </div>
  <!-- ***** Preloader End ***** -->

  <!-- ***** Header Area Start ***** -->
  <header class="header-area header-sticky">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <nav class="main-nav">
                    <!-- ***** Logo Start ***** -->
                    <a href="index.html" class="logo">
                        <img src="assets/images/logo.png" alt="">
                    </a>
                    <!-- ***** Logo End ***** -->
                    <!-- ***** Search End ***** -->
                    <div class="search-input">
                      <form id="search" action="#">
                        <input type="text" placeholder="Search Friends" id='searchText' name="searchKeyword" onkeypress="handle" />
                        <i class="fa fa-search"></i>
                      </form>
                    </div>
                    <!-- ***** Search End ***** -->
                    <!-- ***** Menu Start ***** -->
                    <ul class="nav">
                        <li><a href="index.php">Home</a></li>
                        <li><a href="team.php" class="active">Friends</a></li>
                        <li><a href="profile.php">Profile <img src="assets/images/profile-header.jpg" alt=""></a></li>
                    </ul>   
                    <a class='menu-trigger'>
                        <span>Menu</span>
                    </a>
                    <!-- ***** Menu End ***** -->
                </nav>
            </div>
        </div>
    </div>
  </header>
  <!-- ***** Header Area End ***** -->

  <div class="container">
    <div class="row">
      <div class="col-lg-12">
        <div class="page-content">

          <!-- ***** Online Friends Start ***** -->
          <div class="row">
            <div class="col-lg-8">
              <div class="online-friends header-text">
                <div class="heading-section">
                  <h4><em>Online</em> Friends</h4>
                </div>
                <div class="owl-features owl-carousel">
                  <div class="item">
                    <div class="thumb">
                      <img src="assets/images/featured-01.jpg" alt="">
                      <div class="hover-effect">
                       <a href="../../IlernaHub-FrontEnd-Portfolio/IlernaHub-Frontend(CON PHP)/profiles/ Juanma089.html"> <h6>View Profile</h6></a>
                      </div>
                    </div>
                    <h4>Juanma089<br><span>800 Followers</span></h4>
                    <ul>
                      <li><i class="fa fa-user-check"></i></li>
                    </ul>
                  </div>
                  <div class="item">
                    <div class="thumb">
                      <img src="assets/images/featured-02.jpg" alt="">
                      <div class="hover-effect">
                        <a href="../../IlernaHub-FrontEnd-Portfolio/IlernaHub-Frontend(CON PHP)/profiles/JairoHG.html"> <h6>View Profile</h6></a>
                      </div>
                    </div>
                    <h4>JairoHG<br><span>900 Followers</span></h4>
                    <ul>
                      <li><i class="fa fa-user-check"></i></li>
                    </ul>
                  </div>
                  <div class="item">
                    <div class="thumb">
                      <img src="assets/images/featured-03.jpg" alt="">
                      <div class="hover-effect">
                        <a href="profiles/Fernando09.html"> <h6>View Profile</h6></a>
                      </div>
                    </div>
                    <h4>Fernando09<br><span>1K Followers</span></h4>
                    <ul>
                      <li><i class="fa fa-user-check"></i></li>
                    </ul>
                  </div>
                  <div class="item">
                    <div class="thumb">
                      <img src="assets/images/featured-01.jpg" alt="">
                      <div class="hover-effect">
                        <a href="profiles/Guille.html"> <h6>View Profile</h6></a>
                      </div>
                    </div>
                    <h4>Guille<br><span>2K Followers</span></h4>
                    <ul>
                      <li><i class="fa fa-user-check"></i></li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          <!-- ***** Online Friends END ***** -->

          <!-- ***** Descubrir Amigos Start ***** -->
            <div class="col-lg-4">
              <div class="discover-friends">
                <div class="heading-section">
                  <h4><em>Discover</em>Accounts </h4>
                </div>
                <ul>
                  <li>
                    <span>01</span>
                    <img src="assets/images/avatar-01.jpg" alt="" style="max-width: 46px; border-radius: 50%; margin-right: 15px;">
                    <h6><i class="fa fa-check"></i> Le Chic</h6>
                    <div class="main-button">
                      <a href="#">Follow</a>
                    </div>
                  </li>
                  <li>
                    <span>02</span>
                    <img src="assets/images/avatar-02.jpg" alt="" style="max-width: 46px; border-radius: 50%; margin-right: 15px;">
                    <h6><i class="fa fa-check"></i> Caramelo</h6>
                    <div class="main-button">
                      <a href="#">Follow</a>
                    </div>
                  </li>
                  <li>
                    <span>03</span>
                    <img src="assets/images/avatar-03.jpg" alt="" style="max-width: 46px; border-radius: 50%; margin-right: 15px;">
                    <h6><i class="fa fa-check"></i> Occo</h6>
                    <div class="main-button">
                      <a href="#">Follow</a>
                    </div>
                  </li>
                  <li>
                    <span>04</span>
                    <img src="assets/images/avatar-04.jpg" alt="" style="max-width: 46px; border-radius: 50%; margin-right: 15px;">
                    <h6><i class="fa fa-check"></i> Antique</h6>
                    <div class="main-button">
                      <a href="#">Follow</a>
                    </div>
                  </li>
                  <li>
                    <span>05</span>
                    <img src="assets/images/avatar-01.jpg" alt="" style="max-width: 46px; border-radius: 50%; margin-right: 15px;">
                    <h6><i class="fa fa-check"></i> Awesome</h6>
                    <div class="main-button">
                      <a href="#">Follow</a>
                    </div>
                  </li>
                </ul>
              </div>
            </div>
          </div>
      <!-- ***** Descubrir Amigos END ***** -->

        </div>
      </div>
    </div>
  </div>

  <!-- ***** Footer START ***** -->
  <footer>
    <div class="container">
      <div class="row">
        <div class="col-lg-12">
          <p>Copyright © 2022 <a href="#">IlernaDaw</a> || All rights reserved. 
        </div>
      </div>
    </div>
  </footer>
<!-- ***** FOOTER END ***** -->
  </body>
</html>
